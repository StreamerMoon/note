...
功能
- 生成扩展的需求分析内容（中文，适用于论文或技术报告）
- 生成多张论文可用的示意图（系统架构、呼叫流程、任务分配、GIS 资源热力图、实时路径/重规划示意、甘特图、部署拓扑、历史热点图、用例图）
- 渲染公式为图片并插入 Word（使用 matplotlib mathtext）
- 输出 Word 文件（.docx）和图像文件夹（report_images/）

依赖 (建议在虚拟环境中安装)
  pip install python-docx matplotlib networkx numpy pillow

说明
- 本脚本在本地运行，生成的 Word 文件名：Mixed_Integer_Emergency_Dispatch_Report.docx
- 若需将图替换为更精确的地图/数据，只需修改生成图的对应函数（绘制位置/热力图等）
- 若你有自定义 Word 模板（.docx），可在 Document() 初始化时加载模板：Document("my_template.docx")
- 运行:
    python generate_emergency_dispatch_report.py
"""
import os
import math
...
def draw_system_architecture(fname):
    ...
    arrow(350, 125, 420, 125)  # call->event
    arrow(700, 125, 790, 125)  # event->dispatch
    arrow(200, 200, 200, 300)  # ASR down to GIS
    arrow(560, 200, 560, 300)  # Event down to Routing
    arrow(980, 200, 980, 300)  # Dispatch down to Dashboard
    arrow(560, 450, 560, 520)  # Routing to HIS
...
def generate_long_content():
    """返回一段约 5000 字的中文文本，用作论文级别的需求分析扩展"""
    # We craft structured sections and paragraphs. Aim for ~5000 Chinese characters.
    parts = []
...
def create_report():
    ...
    latex2 = r"u_{v,j} \ge u_{v,i} + s_i + t_{ij} - M(1-x_{v,ij})"
